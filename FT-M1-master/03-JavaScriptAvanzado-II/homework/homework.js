...
function cacheFunction(cb) {
    ...
    return function(arg){
        if (cache.hasOwnProperty(arg)){
          //veo si el argumento está en el objeto
           return cache[arg];
        }
        else{
           cache[arg] = cb(arg);
           //si no está presente lo guardo y devuelvo
            return cache[arg];
        }
    }
}
  ...
  para el callback que recibe por parámetro (cb); es decir, 
  ...
  de esa "memoria" sin tener que efectuar otra vez cálculos que ya se 
  ...
  squareCache(5)    // invocará a square(5), 
  almacenará el resultado y lo retornará
